...
gwet.ac1.raw <- function(ratings,weights="unweighted",categ.labels=NULL,conflev=0.95,N=Inf){ 
  ...
  if (q>=2){pe <- sum(weights.mat) * sum(pi.vec*(1-pi.vec)) / (q*(q-1))}else pe=(1-1e-15)
  gwet.ac1 <- (pa-pe)/(1-pe)
  ...
  df.out <- data.frame(coeff.name,pa,pe,coeff.val,coeff.se,conf.int,p.value,w.name)
  return(list("est" = df.out,"weights" = weights.mat,"categories"=categ))
  #return(c("Gwet's AC1/AC2",pa,pe,gwet.ac1.est,stderr.est,conf.int,format(p.value,digits=6,scientific = TRUE)))
}
...
fleiss.kappa.raw <- function(ratings,weights="unweighted",categ.labels=NULL,conflev=0.95,N=Inf){ 
  ...
  df.out <- data.frame(coeff.name,pa,pe,coeff.val,coeff.se,conf.int,p.value,w.name)
  return(list("est" = df.out,"weights" = weights.mat,"categories"=categ))
  #invisible(c("Fleiss' Pi",pa,pe,fleiss.kappa.est,stderr.est,conf.int,format(p.value,digits=6,scientific = TRUE)))
}
#=====================================================================================
...
krippen.alpha.raw <- function(ratings,weights="unweighted",categ.labels=NULL,conflev=0.95,N=Inf){ 
  ...
  if (q>=2){
    ...
    # calculating variance, stderr & p-value of gwet's ac1 coefficient
    pa.ivec <- sum.q/(ri.mean*(ri.vec-1)) - pa*(ri.vec-ri.mean)/ri.mean
  ...
  df.out <- data.frame(coeff.name,pa,pe,coeff.val,coeff.se,conf.int,p.value,w.name)
  return(list("est" = df.out,"weights" = weights.mat,"categories"=categ))
  #invisible(c("Krippendorff's Alpha",pa,pe,krippen.alpha.est,stderr.est,conf.int,format(p.value,digits=6,scientific = TRUE)))
}
#===========================================================================================
...
conger.kappa.raw <- function(ratings,weights="unweighted",categ.labels=NULL,conflev=0.95,N=Inf){ 
  ...
  df.out <- data.frame(coeff.name,pa,pe,coeff.val,coeff.se,conf.int,p.value,w.name)
  return(list("est" = df.out,"weights" = weights.mat,"categories"=categ))
#  invisible(c("Conger's Kappa",pa,pe,conger.kappa.est,stderr.est,conf.int,format(p.value,digits=6,scientific = TRUE)))
}
#===========================================================================================
...
bp.coeff.raw <- function(ratings,weights="unweighted",categ.labels=NULL,conflev=0.95,N=Inf){ 
  ...
  df.out <- data.frame(coeff.name,pa,pe,coeff.val,coeff.se,conf.int,p.value,w.name)
  return(list("est" = df.out,"weights" = weights.mat,"categories"=categ))
  #invisible(c("Brennan-Prediger",pa,pe,bp.coeff.est,stderr.est,conf.int,format(p.value,digits=6,scientific = TRUE)))
}
#
